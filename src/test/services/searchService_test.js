    ...
    return {
        ...
        execute: function() {
            testing_1.describe('Serch service', function () {
                ...
                testing_1.it('should return empty array if no suggestion', testing_1.inject([search_1.SearchService, testing_2.MockBackend], function (service, mockBackend) {
                    mockHelper_1.HttpMockHelper.injectResponse(mockBackend, '{"Suggestions":[]}');
                    var res = service.getSuggestions('robe');
                    testing_1.expect(res).toEqual([]);
                    testing_1.expect(res.length).toEqual(0);
                }));
            });
        }
    }
